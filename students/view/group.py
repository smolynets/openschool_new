...
def groups_add(request):
  ...
  if request.method == "POST":
    ...
    if request.POST.get('add_button') is not None:
      ...
      if not title:
        errors['title'] = _(u"Name of class is mandatory.")
      else:
        data['title'] = title
      
      data['user_g'] = current_user
      # save student
      if not errors:
        group = Group(**data)
        group.save()
        # redirect to students list
        return HttpResponseRedirect( u'%s?status_message=%s' % (reverse('groups'), _(u'class added sucessfully!')))
      else:
      ...
      return HttpResponseRedirect( u'%s?status_message=%s' % (reverse('groups'), _(u'adding class canceled')))
  else:
   # initial form render
   return render(request, 'students/groups_add_edit.html',
      ...
      if request.POST.get('add_button') is not None:
        ...
        if not leader:
          ...
          try:
            ...
            return HttpResponseRedirect( u'%s?status_message=%s' % (reverse('groups'), _(u'Editing of class canceled. Class hasnt pupils')))
        ...
        if not errors:
          ...
          data.save()
          # redirect to students list
          return HttpResponseRedirect( u'%s?status_message=%s'  % (reverse('groups'), _(u'class edited succefully')))
        else:
          # render form with errors and previous user input
          return render(request, 'students/groups_add_edit.html',
        ...
        # redirect to home page on cancel button
        return HttpResponseRedirect( u'%s?status_message=%s' % (reverse('groups'), _('Editing class canceled')))
    else:
     # initial form render
     return render(request, 'students/groups_add_edit.html',
...
def groups_delete(request, pk):
    ...
    if request.method == "POST":
        if request.POST.get('yes') is not None:
          try:
            groups.delete()
            return HttpResponseRedirect( u'%s?status_message=%s'  % (reverse('groups'), _(u'Class deleted succefully')))
          except:
            return HttpResponseRedirect( u'%s?status_message=%s'  % (reverse('groups'), _(u'Remove impossible because in the class are pupils. You must first remove pupils')))
        elif request.POST.get('cancel_button') is not None:
          return HttpResponseRedirect( u'%s?status_message=%s'  % (reverse('groups'), _(u'Deleting class canceled')))
